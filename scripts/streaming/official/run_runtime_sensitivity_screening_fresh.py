...
from ids_platform.streaming.config.calibration import (  # noqa: E402
    build_capacity_runtime_sensitivity_screening_config,
)
...
SUMMARY_CSV = resolve_project_path(
    "artifacts/streaming/evaluation/capacity_runtime_sensitivity_screening.csv"
)
PARTS_DIR = resolve_project_path(
    "artifacts/streaming/evaluation/runtime_sensitivity_screening_parts"
)
...
def main() -> int:
    if len(sys.argv) > 1:
        raise SystemExit(
            "run_runtime_sensitivity_screening_fresh.py is config-driven and accepts no CLI arguments."
        )
...
def _run_single_process(run_index: int) -> int:
    ...
    config = build_capacity_runtime_sensitivity_screening_config(summary_csv=summary_csv)
...
def _run_fresh_process_matrix() -> int:
    config = build_capacity_runtime_sensitivity_screening_config(
    ...
    base_namespace = safe_tag(
        ...
        or "runtime_sensitivity_screening"
    )

...
from ids_platform.streaming.config.calibration import (  # noqa: E402
    build_capacity_runtime_sensitivity_candidate_confirmation_config,
)
...
def main() -> int:
    if len(sys.argv) > 1:
        raise SystemExit(
            "run_runtime_sensitivity_candidate_confirmation_fresh.py is config-driven "
...
def _summary_csv() -> Path:
    ...
    return resolve_project_path(
        ...
        f"capacity_runtime_sensitivity_candidate_confirmation_{stamp}.csv"
    )
...
def _run_single_process(run_index: int) -> int:
    ...
    config = build_capacity_runtime_sensitivity_candidate_confirmation_config(
...
def _run_fresh_process_matrix() -> int:
    ...
    base_namespace = safe_tag(
        ...
        or f"runtime_sensitivity_candidate_{summary_csv.stem}"
    )
    print(

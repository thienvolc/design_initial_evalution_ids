...
def main() -> int:
    if len(sys.argv) > 1:
        raise SystemExit(
            "run_overload_degradation_reproducible.py is config-driven "
...
def _summary_csv() -> Path:
    ...
    return resolve_project_path(
        f"artifacts/streaming/evaluation/overload_degradation_3run_{stamp}.csv"
    )
...
def _run_single_process(run_index: int) -> int:
    ...
    config = build_overload_degradation_main_config()
    ...
    print(
        ...
        f"profile={single_run.summary_context.get('overload_profile', '') if single_run.summary_context else ''} "
        f"rps={single_run.summary_context.get('target_rps', '') if single_run.summary_context else ''} "
        f"traffic={single_run.summary_context.get('traffic_mode', '') if single_run.summary_context else ''}",
        flush=True,
    )
    return int(run_overload_degradation_matrix(single_config))
...
def _run_fresh_process_matrix() -> int:
    ...
    base_namespace = safe_tag(
        ...
        or f"overload_degradation_{summary_csv.stem}"
    )
    print(
    ...
    for run_index, run_plan in enumerate(config.runs, start=1):
        ...
        print(
            ...
            f"repeat={benchmark.repeat_index} "
            f"profile={run_plan.summary_context.get('overload_profile', '') if run_plan.summary_context else ''} "
